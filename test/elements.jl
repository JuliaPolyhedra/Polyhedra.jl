...
@testset "Element" begin
    ...
    @testset "Simplify" begin
        r = @inferred Polyhedra.simplify(Ray([0, 0]))
        @test r isa Ray{Int}
        @test coord(r) == [0, 0]
        p = @inferred Polyhedra.simplify([6, 4])
        @test p isa Vector{Int}
        @test p == [6, 4]
        l = @inferred Polyhedra.simplify(Line([6, 4]))
        @test l isa Line{Int}
        @test coord(l) == [3, 2]
        hp = @inferred Polyhedra.simplify(HyperPlane([6, 4], 2))
        @test hp isa HyperPlane{Int}
        @test hp.a == [3, 2]
        @test hp.β == 1
        hs = @inferred Polyhedra.simplify(HalfSpace([0, 0], 0))
        @test hs isa HalfSpace{Int}
        @test hs.a == [0, 0]
        @test hs.β == 0
        hpr = @inferred Polyhedra.simplify(HyperPlane([0//1, 0//1], 0//1))

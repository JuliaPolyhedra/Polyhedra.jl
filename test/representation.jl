...
@testset "Representation tests" begin
    ...
    @testset "eltype for some iterators is incorrect #7" begin
        ...
        function htest(hr::Polyhedra.HRepresentation, AT::Type{<:AbstractVector})
            @test (@inferred coefficient_type(hr)) == Float64
        ...
        function vtest(vr::VRepresentation, AT::Type{<:AbstractVector})
            @test (@inferred coefficient_type(vr)) == Int
...
#        vtest((@inferred vrep(sps, sls, srs)), SVector{2, Int})
    ...
    @testset "Building rep with different type" begin
        @test coefficient_type(MixedMatHRep{Float64}([1 2; 3 4], [1, 2], BitSet())) == Float64
        @test coefficient_type(MixedMatVRep{Float64}([1 2; 3 4], [1 2; 3 4], BitSet())) == Float64
        @test coefficient_type(LiftedHRepresentation{Float64}([1 2; 3 4])) == Float64
        @test coefficient_type(LiftedVRepresentation{Float64}([1 2; 3 4])) == Float64
    end

...
"""
    struct Ray{N, T, AT <: AbstractVector{N, T}}
        a::AT
    end
...
struct Line{N, T, AT<:AbstractVector{N, T}}
    a::AT
    function Line{N, T, AT}(a::AT) where {N, T, AT<:AbstractVector{N, T}}
        new{N, T, AT}(a)
    end
end
...
Line{N, T}(a::AT) where {N, T, AT<:AbstractVector{N, T}} = Line{N, T, AT}(a)

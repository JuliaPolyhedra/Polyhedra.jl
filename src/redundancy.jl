...
# Redundancy
export detecthlinearity!, detectvlinearity!, dim, detectvlinearity
...
function detect_new_lines(lines, rays, solver)
    ...
    for i in 1:n # safer than `while ...`
        count(is_lin) == length(is_lin) && break
        Polyhedra.is_feasible(model, "detecting new linearity.") || break
        value = MOI.get(model, MOI.VariablePrimal(), λ)
        I = findall(i -> !is_lin[i] && !Polyhedra.isapproxzero(value[i]), eachindex(value))
        if isempty(I)
            error("The values `$(value[map(!, is_lin)])` should sum up to one but they are all zero.")
        end
        for i in I
